...
void DrawQueue::correctOutfit(const Rect& dest, int fromPos, bool ui, bool oldScaling, float scale)
{
    ...
    if (!ui || oldScaling) {
        ...
        const int dWidth = dest.width() / scale;
        const int dHeight = dest.height() / scale;
        for (auto& rect : rects) {
            int x = rect->left() - x1 + (dWidth - (x2 - x1));
            int y = rect->top() - y1 + (dHeight - (y2 - y1)); // offset
            *rect = Rect(dest.left() + x * scale, dest.top() + y * scale, rect->size() * scale);
        }
    }
}
